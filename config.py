...
# === v2.1 Phase 2-B/2-C — 360° 둘러보기 모드 파이프라인 (동기 캡처·중복 제거·문장 조립) ===
...
#
# ⚠️ 2026-08-14~16에 이 누적·요약 방식을 "실시간으로 대상이 바뀔 때마다 말하는" 2-E로
# 대체했었다가, 2026-08-22 실사용 결과 회전 중 계속 말이 끼어드는 쪽이 더 불편해
# 이 계획 A(버튼을 다시 누를 때 한 번에 요약)로 되돌렸다.
...
# 한 방향에서 말할 최대 그룹 수. 발화는 방향(앞→오른쪽→뒤→왼쪽)으로 묶여 나가므로
# 상한도 방향별로 건다 — 전체 상한 하나(2026-08-25까지 SCAN_MAX_ANNOUNCE_ITEMS=8)로는
# 물체가 몰린 방향 하나가 정원을 다 써서 다른 방향이 통째로 사라질 수 있었다
# (앞 6그룹 + 오른쪽 1 + 뒤 1 = 왼쪽이 있어도 한 마디도 안 나감). 방향별로 나눠 담으면
# 어느 방향도 통째로 빠지지 않는다. 최대 4방향 × 3 = 12그룹.
SCAN_MAX_ITEMS_PER_DIRECTION: int = 3
